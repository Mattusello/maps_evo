...
export function StopDetailSheet({ stop, currency, userId, onDismiss, onRemove }: Props) {
  ...
  const crowdTone = crowd?.level === 'basso' ? 'success' : crowd?.level === 'medio' ? 'warning' : 'danger';

  const savePrice = async () => {
    if (!stop) return;
    const value = parseFloat(amount.replace(',', '.'));
    if (!Number.isFinite(value) || value < 0) return;
    await priceRepo.add({ stopId: stop.id, price: { amount: value, currency }, reportedBy: userId });
    setAmount('');
    setAdding(false);
            ...
            {crowd ? (
              <View style={styles.section}>
                ...
                <Badge
                  tone={crowdTone}
                  dot
                  label={t(`crowd.${crowd.level === 'basso' ? 'low' : crowd.level === 'medio' ? 'medium' : 'high'}`)}
                />
